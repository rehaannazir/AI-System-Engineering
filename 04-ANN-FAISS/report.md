...
| Index | Corpus Size | Recall | Latency (ns) | Reason |
|---|---|---|---|---|
| `IndexFlatL2` (exact) | 100,000 | 5/5 | 757,547,617 | Brute-force scan over all 100k × 768 vectors — always exact, but cost scales linearly with corpus size, so it's now the slowest of the three. |
| `IndexIVFFlat` (nlist=100, nprobe=8) | 100,000 | 0/5 | 190,907,240 | Only searches 8 of 100 clusters (~8% of the corpus). On this run none of the true top-5 fell into a visited cluster — with uniformly random 768-dim data, distances barely vary between points, so `nprobe=8` has no reliable signal for which clusters actually hold the nearest neighbors. |
| `IndexHNSWFlat` (M=32, efSearch=128) | 100,000 | 2/5 | 869,407,415 | Recall improved from 0/5 (at the default `efSearch=16`) to 2/5 after raising `efSearch` to 128, confirming the earlier miss was a tuning issue — but it cost roughly 27x the search latency of the `efSearch=16` run, and recall still isn't close to Flat's exactness on this adversarial (uniform random) dataset. |

**Takeaway:** raising `efSearch` did exactly what it's supposed to — traded speed for recall, and recall went up. But it's still far from perfect, which reinforces the earlier point: uniform random vectors in 768 dimensions are close to worst-case for ANN search, since distances between points concentrate and there's little real neighborhood structure for IVF's clusters or HNSW's graph to exploit. On real embeddings (like the Gemini-generated ones used above), these same indexes typically reach much higher recall at far lower `nprobe`/`efSearch` settings, because genuine semantic data has actual clusters for these structures to find.
